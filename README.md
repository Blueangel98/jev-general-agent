...
Browser synthesis is incremental: tasks with `M0 — ...`, `M1 — ...` milestones (or three or more Markdown headings) are split into bounded Codex-like implementation steps. JEV opens a fresh temporary chat for each step, keeps enough task and workspace context for a coherent change, collects one JSON patch response, and only then assembles the candidate for the normal sandbox and live verification gates. This prevents one oversized prompt/answer from carrying the entire project at once.
